...
/* Client ids are Date.now()*1000 + a random suffix, so roughly 1.8e15 — comfortably
   inside MAX_SAFE_INTEGER but well past a 1e15 cap. Capping below the id range would
   clamp every id to the same number and merge the whole log into one entry. */
const ID_MAX = 9e15;
...
/* Where the trip was actually fished. The docks sit inside the canal, so they share
   the canal's conditions for scoring but are recorded separately — the point is to
   be able to tell later whether they fish differently. */
const SPOTS = ['canal', 'scotts', 'ryans', 'shelf'];
const MODEL_SPOTS = ['canal', 'shelf'];
...
function cleanTrip(t) {
  ...
  const id = num(t.id, 1, ID_MAX, 0);
  if (!id) return null;
  return {
    id,
    when: str(t.when, 40),
    edited: num(t.edited, 0, ID_MAX, 0),      // last-write-wins marker for edits
    spot: SPOTS.includes(t.spot) ? t.spot : 'canal',
    modelSpot: MODEL_SPOTS.includes(t.modelSpot) ? t.modelSpot : 'canal',
...
function mergeById(existing, incoming, clean, cap, tomb) {
  ...
  for (const item of (incoming || []).slice(0, MAX_BATCH)) {
    const c = clean(item);
    if (!c || dead.has(c.id)) continue;
    const prev = out.get(c.id);
    if (!prev) { out.set(c.id, c); added++; }
    /* An edit carries a newer `edited` stamp. Without this the merge would keep the
       original forever and edits would silently never leave the device. */
    else if ((c.edited || 0) > (prev.edited || 0)) { out.set(c.id, c); changed++; }
  }
  let arr = [...out.values()].sort((a, b) => a.id - b.id);
  if (arr.length > cap) arr = arr.slice(arr.length - cap);
  return { arr, added, changed };
}
...
export default async (req) => {
  ...
  if (req.method === 'POST') {
    ...
    return json({ ...current, addedTrips: t.added, editedTrips: t.changed, addedReadings: r.added, deletedOk: !!wantsDelete });
  }
